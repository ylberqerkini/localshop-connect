...
export default function ProductDetail() {
  ...
  useEffect(() => {
    async function load() {
      ...
      // Fetch related products (same category or same business)
      const { data: related } = await supabase
        .from('products')
        .select('id, name, price, description, image_url, stock_quantity, category_id, business_id')
        .eq('business_id', biz.id)
        .eq('is_active', true)
        .neq('id', productId)
        .limit(4);
      ...
      <div className="min-h-screen flex flex-col items-center justify-center bg-background gap-4">
        ...
        <Link to={`/store/${subdomain}`}>
          <Button variant="outline" className="gap-2">
            <ArrowLeft className="h-4 w-4" /> Kthehu në dyqan
          </Button>
        </Link>
      </div>
    );
          ...
          <div className="flex flex-col gap-4">
            ...
            {product.stock_quantity !== null && (
              <p className="text-sm text-muted-foreground">
                {outOfStock ? (
                  <span className="text-destructive font-medium">Pa stok</span>
                ) : (
          ...
          <section className="mt-12 max-w-4xl mx-auto">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              {relatedProducts.map(rp => (
                <Link
                  key={rp.id}
                  to={`/store/${subdomain}/product/${rp.id}`}
