...
const navItems = [
  ...
  { path: '/dashboard/coupons', label: 'Kuponat', icon: Ticket },
  { path: '/dashboard/reviews', label: 'Vlerësimet', icon: Star },
        ...
        <div className="flex flex-col h-full">
          ...
          {business && (
            <div className="px-4 py-3 border-b">
              <div className="bg-muted/50 rounded-lg p-3">
                ...
                <Link to={`/store/${business.subdomain}`} target="_blank"
                  className="text-xs text-muted-foreground hover:text-primary flex items-center gap-1 mt-1">
                  {business.subdomain}.eblej.com <ExternalLink className="h-3 w-3" />
                </Link>
              </div>
            </div>
          )}
          ...
          <div className="p-4 border-t">
            <Button variant="ghost" className="w-full justify-start gap-3 text-muted-foreground hover:text-destructive" onClick={handleSignOut}>
              <LogOut className="h-5 w-5" /> Dil nga llogaria
            </Button>
          </div>
        </div>
      </aside>
